...
namespace VRCTools.World.LocalValues.UI {
  ...
  [AddComponentMenu("Local Values/UI/Local URL Label")]
  public class LocalUrlLabel : UdonSharpBehaviour {
    ...
    private void Start() {
      if (!Utilities.IsValid(this.localValue)) {
        Debug.LogError("[Local URL Label] Local value is invalid - Disabled", this);
        this.enabled = false;
        return;
      }
