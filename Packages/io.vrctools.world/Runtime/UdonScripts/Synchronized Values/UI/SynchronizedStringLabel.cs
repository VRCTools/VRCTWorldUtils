...
namespace VRCTools.World.SynchronizedValues.UI {
  ...
  [AddComponentMenu("Synchronized Values/UI/Synchronized String Label")]
  public class SynchronizedStringLabel : UdonSharpBehaviour {
    ...
    private void Start() {
      if (!Utilities.IsValid(this.synchronizedValue)) {
        Debug.LogError("[Synchronized String Label] Synchronized value is invalid - Disabled", this);
        this.enabled = false;
        return;
      }
